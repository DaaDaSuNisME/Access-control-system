...
{    
	...
	delay_init();
	//TIM4_1sConfig();
	TIM5_100msConfig();//生成时长为1s的延迟
	LED_Init();
	Esp_Init();
	LED_Init();
	TIM3ocConfig();//为舵机生成PWM
	
	//ADCr_Config();  //自带电阻ADC测试
	//TIM3_Int_Init(7199,9999); //ADC2 测试
	//TIM2ocConfig(7199,39999);//比较输出4路PWM
	delay_ms(2000);
	Espconnected();
	...
	while(1)
	{
		...
		if(GPIO_ReadOutputDataBit( GPIOB,GPIO_Pin_5)==RESET&&flagR==0){
			...
			Esp_SendChar("#0,1,0,0#", 11);		flagG++;}//'\0'占两个字节，回车符'/n'也占用两个字符
